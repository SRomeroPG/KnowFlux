...
| T04 §§4.1, 10.5 | BP, BR and DE are faceted; CAP, OUT, SC, EV, UNK, CHG, DIV and DSP SHALL NOT be faceted. | entity and facet schemas | positive and negative facet fixtures | PASS |
...
| T08 §§7.1–7.2 | Every authored edge has one owner; inverse edges SHALL NOT appear in authored YAML, including BR `evidence` and `executed_by`. | relationship definitions in entity/facet schemas | inverse matrix negatives; endpoint-shaped positives | PASS |
| T09 §7.1 | BP execution edges are authored within the corresponding BP facet; `part_of` points BP→CAP. | BP entity/facet schemas | BP facet and wrong-level edge cases | PASS |
| T10 §7.1 | DE `contains`, SC `exercises`/`covers`, EV `supports`, UNK `blocks`, CHG `affects`, DSP `disputes` retain canonical owners. | respective core schemas | relationship owner cases | PASS |
...
| T19 §§10.8, 19.1 | `created_at` and `updated_at` SHALL NOT exist in YAML metadata. | common schema exclusion | root and facet negatives | PASS |
...
| T22 §§16.2–16.3 | EV SHALL declare `assertion_kind: fact | interpretation`; supports SHALL address a facet for a faceted entity. | EV schema and reference definitions | missing/invalid kind and bare faceted support negatives | PASS |
...
| T29 §23.3 | DIV SHALL NOT duplicate claims via `statement`; its persistent `detected` and `disposition` sections are distinct. | DIV schema | synthetic derived DIV positive; statement negative | PASS |
...
The scenario-count contradiction between §7.3 R3 and §26.2 is tracked as [SG-0001](../../spec-gaps/SG-0001.md), affecting P007/P014 and not resolved by P001.
